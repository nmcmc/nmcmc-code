...
from phys_models.U1 import U1GaugeAction
...
def Hopping(u, device, *, float_dtype=torch.complex64):
    ...
    L = u.shape[-1]
    M = torch.zeros((u.shape[0], 2 * L * L, 2 * L * L), dtype=float_dtype, device=device)
    ...
    for x0 in range(0, L):
        for x1 in range(0, L):
            ...
            y = L * y1 + y0
            assert (x != y)
            M[:, 2 * y:2 * y + 2, 2 * x:2 * x + 2] = sign * (id + pauli[0]) * torch.exp(
                -1.0j * u[:, 0, x1, x0]).reshape(-1, 1, 1)
            sign = 1
            y1 = x1 - 1
            if y1 < 0:
                y1 += L
...
def calc_condensate(cfgs, *, kappa, batch_size, device, float_dtype=torch.complex64):
    return calc_function(cfgs, batch_size=batch_size,
                         function=lambda cfg: condensate(cfg, kappa, device=device, float_dtype=float_dtype),
                         device=device)
